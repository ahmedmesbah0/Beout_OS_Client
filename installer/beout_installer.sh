...
EOF
    ...
    # Reset network interfaces to a clean, minimal state to prevent boot/restart failures
    mkdir -p "${TARGET_MNT}/etc/network/interfaces.d"
    cat <<EOF > "${TARGET_MNT}/etc/network/interfaces"
...
EOF
    echo "# Generated by Beout_OS Configuration Engine" > "${TARGET_MNT}/etc/network/interfaces.d/beout_os_interfaces"
